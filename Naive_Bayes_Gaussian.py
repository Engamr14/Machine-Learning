...
def Compute_Diagonal_Covariance (X,mu):
    ...
    Cov /= X.shape[1]
    de = Cov.diagonal()
    DiagCov = np.zeros((Cov.shape[0], Cov.shape[1]))
    np.fill_diagonal(DiagCov, de)
...
class Naive_Bayes_Gaussian:
    ...
    def fit(self, DTR, LTR):
        ...
        for clsno in self.difClasses:
            ...
            self.cov.append(Compute_Diagonal_Covariance(self.D[clsno],self.muC[clsno]))
